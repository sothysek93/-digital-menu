<template>
  <div class="space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <Card v-for="shop in shops" :key="shop.id" class="overflow-hidden">
        ...
        <CardContent>
          <div class="text-xs text-muted-foreground mb-4">
             <div class="flex items-center gap-1">
               <LucideGlobe class="h-3 w-3" /> /{{ shop.slug }}
             </div>
          </div>
          
          <div class="space-y-1 text-sm">
             <p class="flex items-center gap-2">
               <LucideMapPin class="h-4 w-4 text-muted-foreground" />
               {{ shop.address || 'No address' }}
             </p>
          </div>

          <div class="flex items-center gap-2 mt-6">
            <Button variant="outline" class="w-full" size="sm" @click="openModal(shop)">Edit</Button>
            <Button variant="outline" size="sm" class="text-destructive hover:bg-destructive/10" @click="deleteShop(shop.id)">
      ...
      <div v-if="!pending && (shops?.length || 0) === 0" class="col-span-full py-20 text-center border-2 border-dashed rounded-lg">
         <LucideStore class="h-10 w-10 text-muted-foreground mx-auto mb-4" />
         <p class="text-muted-foreground">No shops found. Start by adding your first location.</p>
      </div>
    </div>
  </div>
</template>
